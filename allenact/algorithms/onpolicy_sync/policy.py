...
class LinearActorHead(nn.Module):
    def __init__(
        ...
        num_outputs: int,
        *,
        action_space: Optional[Union[gym.spaces.MultiDiscrete, gym.spaces.Discrete]],
    ):
        super().__init__()
        self.action_space = action_space
        self.is_multi_discrete = (
            action_space is not None and len(action_space.shape) != 0
        )
    ...
    def forward(self, x: torch.FloatTensor):  # type: ignore
        ...
        # noinspection PyArgumentList
        return (
            CategoricalDistr(logits=x)
            if not self.is_multi_discrete
            else (
                MultiCategorical(logits=x, action_dim=self.action_space.nvec.tolist())
            )
        )  # logits are [step, sampler, ...]
