...
from allenact.utils.viz_utils import VizSuite, TrajectoryViz
...
class OnPolicyRunner(object):
    ...
    def process_valid_package(
        ...
        for callback in self.callbacks:
            extra_vizs = []
            if self.visualizer is not None and self.visualizer.viz is not None:
                for viz in self.visualizer.viz:
                    if isinstance(viz, (TrajectoryViz, ThorViz)):
                        extra_vizs.append(
                            viz.maybe_log(
