...
class TrajectoryViz(AbstractViz):
    ...
    def log(
        ...
        self.maybe_log(log_writer, task_outputs, render, num_steps, log=True)
    ...
    def maybe_log(
        ...
        if viz_order is None:
            get_logger().warning("trajectory viz returning without visualizing")
            return
        viz_figs = dict()
        for page, current_ids in enumerate(viz_order):
            ...
            for episode_id in current_ids:
                ...
                if episode_id not in all_episodes:
                    ...
                    continue
                fig = self.make_fig(all_episodes[episode_id], episode_id)
                figs.append(fig)
            if len(figs) == 0:
                continue
            viz_figs["{}/{}_group{}".format(self.mode, self.label, page)] = figs
            if log:
        ...
        if len(vector_task_data) > 0:
            for (
                ...
                if not isinstance(res, Sequence):
                    assert len(it2epid) == 1, (res.shape, it2epid)
                    res = [res]
                if method == "render":
