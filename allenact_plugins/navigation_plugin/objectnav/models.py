...
from allenact_plugins.models.nn.mlp import one_layer_state_encoder
...
class ResnetTensorGoalEncoder(nn.Module):
    def __init__(
        ...
        def _maybe_compress():
            ...
            if not self.blind:
                # (x, oh, ow) if not pooled, (x, ) if pooled
                self.resnet_tensor_shape = observation_spaces.spaces[
                    self.resnet_uuid
                ].shape
                if len(self.resnet_tensor_shape) != 1:
                    self._compress = True
                    ...
                    get_logger().warning(f"not used")
                    # self.resnet_compressor = nn.Identity()
                    if self.resnet_tensor_shape[-1] == 2048:
                        self.resnet_compressor = nn.Sequential(
                            nn.Linear(self.resnet_tensor_shape[-1], 1024),
                        ...
                        self.resnet_compressor = nn.Sequential(
                            nn.ReLU(inplace=True),
                            *one_layer_state_encoder(self.resnet_tensor_shape[-1], 512),
                        )
                    self.target_obs_combiner = nn.Identity()
    ...
    def output_dims(self):
        if self.blind:
            ...
            return (
                ...
                if self._compress
                # TODO configurable
                else (self.combine_hid_out_dims[-1] + 512)
            )
    ...
    def distribute_target(self, observations):
        ...
        return (
            target_emb.view(-1, self.goal_embed_dims, 1, 1).expand(
    ...
    def forward(self, observations):
        ...
        cat_dim = 1 if self._compress else -1
        x = self.target_obs_combiner(torch.cat(embs, dim=cat_dim))
