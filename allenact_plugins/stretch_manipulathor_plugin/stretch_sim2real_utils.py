...
__all__ = [
    "kinect_reshape",
    "intel_reshape",
    "clip_depth_kinect_frame",
    "set_mask_kinect_converted_depth",
]
...
def clip_depth_kinect_frame(frame):
    ...
    if len(frame.shape) == 3:
        w, h, c = frame.shape
    if (
        DEPTH_KINECT_MASK_FRAMES is None
        or DEPTH_KINECT_MASK_FRAMES.shape[0] != w
        or DEPTH_KINECT_MASK_FRAMES.shape[1] != h
        or random.random() < remake_mask_prob
    ):
...
def set_mask_kinect_converted_depth(w, h):
    ...
    init = [
        (w_left_border, 0),
        (original_w - w_right_border, 0),
        (w_left_border, original_h),
        (original_w - w_right_border, original_h),
    ]
...
def tweak_mask_values(list_to_be_tweaked, value_to_change=PIX_NOISE):
    return [
        x + random.randint(-value_to_change, value_to_change)
        for x in list_to_be_tweaked
    ]
