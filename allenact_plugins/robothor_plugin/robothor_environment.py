...
class RoboThorEnvironment:
    ...
    def set_object_filter(self, object_ids: List[str], render_image: bool = False):
        self.controller.step(
            "SetObjectFilter", objectIds=object_ids, renderImage=render_image
        )

    def reset_object_filter(self, render_image: bool = False):
        self.controller.step("ResetObjectFilter", renderImage=render_image)
    ...
    def distance_to_object_type(self, object_type: str, agent_id: int = 0) -> float:
        ...
        agent_position = self.controller.last_event.events[agent_id].metadata["agent"][
            "position"
        ]
        distance = self.distance_cache.find_distance(
            self.scene_name, agent_position, object_type, retry_dist,
        )
        if distance < 0:
            get_logger().debug(
                f"In scene {self.scene_name}, could not find a path from {agent_position} "
                f"to {object_type}. Agent state: {self.agent_state(agent_id)} Returning a distance of -1."
            )
        return distance
    ...
    def path_from_point_to_point(
        ...
        try:
            ...
            get_logger().debug(
                "Failed to find path for {} in {}. Start point {}, agent state {}. Exception {}".format(
                    target,
